...
Until an AsaDB version is published in the public SWI-Prolog pack registry,
the direct Git command is available for development and testing:
...
The direct `--git` form follows the repository branch, so it is not the
release-following channel for production use. Use the registered `asadb` name
for installs that must update only to published stable versions.
